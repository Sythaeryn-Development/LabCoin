...
static const int64_t nTargetTimespan = 4 * 60 * 60; // Labcoin: every 4 hours
...
bool CBlock::AcceptBlock()
{
    ...
    // Unfortunately there are bad version 2 blocks in Labcoin blockchain (e.g. block 6)
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        if (wallet.CreateCoinStake(wallet, nBits, nSearchTime-nLastCoinStakeSearchTime, nFees, txCoinStake, nTxTime, key))
        {
            ...
            {
                // Labcoin: since I've had to get rid of CTransaction's nTime,
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Labcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
